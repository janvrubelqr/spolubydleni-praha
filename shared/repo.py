...
Použití:
    ...
    repo.finish_run(run_id, status='ok', items_seen, items_new, items_updated)
"""
...
class SupabaseRepo:
    def __init__(self, url: Optional[str] = None, service_key: Optional[str] = None):
        ...
        if not url or not service_key:
            raise RuntimeError(
                "SUPABASE_URL a SUPABASE_SERVICE_KEY nejsou nastaveny. "
                "Vytvořte soubor .env nebo .env.local se správnými hodnotami, "
                "nebo je exportujte do prostředí."
            )
